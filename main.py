...
def get_inv(store_id: Optional[int] = None):
    ...
    p = []
    if store_id: q += " WHERE i.store_id = ?"; p.append(store_id)
    return success([dict(r) for r in get_db().execute(q, p).fetchall()])
...
def io(storeId: Optional[int] = None):
    ...
    p = []
    if storeId: q += " WHERE s.id=?"; p.append(storeId)
    q += " GROUP BY s.id"
    return success([dict(r) for r in get_db().execute(q, p).fetchall()])
